...
# EXPANSIONS
  ...
  2. **Safari Zone Rotation**: 1 unique pokemon in each area, per day!
  3. **Prof. Birch Research:** Each day, Prof. Birch's assistant requests a new Pokémon to be submitted. Rewards scale with Pokémon requested.
  4. **Capt. Stern's Expeditions (POST-Game):** Fund expeditions to locate the event islands tied to Legendary Pokémon.
  5. **Legendary Rumors:** Each day, a Legendary is rumored to appear in specific locations.
  6. **Expanded Outbreaks (GYM 5):** Outbreaks are fixed to occur more than once. Additionally, has a chance to make 1% encounters on routes appear in the outbreak.
  7. **Natural Evolution:** All wild Pokémon have a chance to be replaced with their evolved counter-part. Even smaller chance for a stage 2 Pokémon instead.
  8. **Rydel:** Utilizes the existing bike time trial on the seaside bicycle path for a chance of PROTO-BIKE.
  9. **Fishing:** Reduces chance of 'nothing' and offers a chance of finding items.
  10. **Rock Smash:** Reduces chance of 'nothing' and offers a chance of finding items.
  11. **Universal Rematches:** All trainers can be rematched at any time. PokéNAV trainer teams advance when they are ready for a rematch via the Match Call system.
  12. **Shiny Symbols:** Each Battle Frontier Gold Symbol increases shiny odds. This stacks until 1/1024 (from 1/8192).
  13. **Pickup:** Expanded Pickup to also include a friendship table, which is rarely called instead of the RNG tables.
  14. **Wonder Corner** Removed TM prizes from Game Corner and instead placed 5 Wonder Pokémon that reset each day with dynamic prices determined by final stage base stats. Can contain any basic pokémon in-game.
  15. **Wonder Egg** Located at the daycare, the Wonder Egg vendor sells the player an egg for 10,000 each day. This egg is guaranteed to have 15 IVs in each stat at minimum. Can contain any basic pokémon in-game.
  16. **Wonder Traders** Located in several locations around Hoenn, several unique "trainers" with differing personalities will trade you Pokémon that are thematic. These are guaranteed to have at LEAST the IVs of the traded Pokémon and will also share the nature.
  17. **Winstrate Family** VITO is now properly a part of the Winstrate Family.
  18. **Wild double battles** Added ability to encounter two wild Pokémon, also fixed the GEN III bugs regarding 1 pokemon in double battles.

# QOL
  Note: These are "always ON" mechanics, not toggles available in the OPTIONS section of this document.
  1. **Bag Sorting**: Press START while in the main items pocket to sort by: NAME, TYPE, VALUE. Works on the SELL menu as well.
  2. **Auto Bag Sorting**: Uses your last filter (default = NAME) to sort the bag automatically.
  3. **PC Sorting**: Press A while on the BOX NAME to sort by: DEX NO., NAME, TYPE, LEVEL, BASE STATS.
  4. **Registered Items**: Can register up to 4 items. SELECT for item 1, START for item 2, "L" for item 3, "R" for item 4.
  5. **Run at Start + Run Indoors:** Run from the moment you start the game and run indoors.
  6. **Catch EXP:** Gain EXP when catching Pokémon too.
  7. **Text Adjustments:** Removes a lot of extra text from actions such as accessing the PC, Healing, Saving, Berry Picking and more.
  8. **BW Repel System:** Allows the user to use another repel when the the active repel runs out.
  9. **Pokémon Summary Screen:** EV Display = START, IV Display = SELECT, Red Stat = Nature increased, Blue Stat = Nature decreased.
  10. **Reusable TMs:** TMs can be used any number of times.
  11. **Erasable HMs:** HMs can be erased without a move deleter.
  12. **Use HMs without Learning:** Allows Pokémon to use HM in overworld if the Pokémon can learn the HM, instead of requiring the Pokémon to know the HM.
  13. **Premier Balls Fixed:** Receive a Premier Ball for each stack of 10 Pokéballs bought.
  14. **Item Move PC:** Move Pokémon screen in PC is given GIVE/TAKE Item commands.
...
# SETTINGS (OVERVIEW)
Pokémon Expanded Emerald comes with several pages of 60+ settings. These are STANDARD, QOL, TIMESAVERS, DIFFICULTY and NUZLOCKE.
...
# Settings - Standard
  ...
  5. Nature Tint: Tints Pokémon based on their nature.
...
# Settings - QoL
  ...
  6. Auto-Run: _On_ Hold "B" to walk.
...
# Settings - Difficulty
  ...
  6. Wild EVO: Toggle if Evolved Wild Pokémon can appear. Replaces an encounter in any given area with it's evolved counterpart(s).
  7. Wild EVO Freq: The rate at which evolved pokémon occur in the wild.
  8. Wild Doubles: Toggle double battles in the wild.
  9. Wild DBL 1 PKMN: Toggle whether doubles can happen if the player only has one pokemon in their party.
  10. Wild DBL Freq: The rate at which wild doubles occur.
  12. Wild Scaling: _OFF_ Wild Pokémon are unchanged. _ON_ Wild Pokémon levels scale to player's level if higher. _ON+EVO_ Wild Pokémon levels and evolution stage scale to player's level.
  13. Enhanced Wild AI: _On_ allows wild Pokémon to make intelligent decisions instead of random.
  14. Trainer Scaling: _OFF_ trainers keep base teams. _GYM_ Trainer level scales based on gym progress. _GYM+_ Trainer level/evolution scale based on gym progress. _ON_ Trainer level scales based on team level. _ON+_ Trainer level/evo scales based on team level.
  15. Trainer Items: _XItems_ disables the opponent's use of heal items but allows battle items. _Off_ disables all opponent items in battle. This does not affect hold items.
  16. Trainer EVs: _Off_ use vanilla values. _Scale_ increases EVs depending on badge count. _Half_ places trainers at 128 EVs. _Full_ places trainers at 252 EVs.
  17. Trainer IVs: _Off_ use vanilla values. _Scale_ increases IVs depending on badge count. _Half_ places trainers at 15 IVs. _Full_ places trainers at 31 IVs.
  18. Trainer IV/EV: For Trainer EVs and Trainer IVs settings, _Challenge_ edits only leaders, rivals, admins, etc. _All_ edits all trainers in-game.
  19. Challenge Teams: Toggle ON/OFF trainer's using enhanced teams for added difficulty.
  20. Difficult AI: _ON_ Trainer AI is updated to be far more intelligent.
  21. Economy: _'value: x'_ increases the buy price of items and decreases the sell price of items by the same amount.
  22. Daycare Price: _'value: x'_ increases the cost of buying a Pokémon back from the daycare by the same amount.
  23. Rematch Lock: _On_ prevents the player from spamming trainers with rematches.
  24. EXP/Catch Scaling: Pokémon IVs and Base Stats affect EXP and catch rate. Level difference between player and opponent Pokémon affects EXP earned.
  25. Poison Survival: _Off_ allows Pokémon to faint from poison in the overworld. _On_ prevents the Pokémon from fainting but leaves the status on the Pokémon. _On+Heal_ prevents the Pokémon from fainting and removes the status when at 1 HP.
  26. WHTOUT Penalty: Toggle ON/OFF whiteout penalties.
  27. WHTOUT Cost: The amount of money lost is equivalent to the trainer earnings multiplied by the chosen multiplier.
